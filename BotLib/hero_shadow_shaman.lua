...
function X.ConsiderQ()
	...
	then
		...
		then
			if nSkillLV >= 2 or nMP > 0.7 or J.GetHPR(botTarget) < 0.5 or nHP < 0.4
			then
...
function X.ConsiderE()
	...
	then
		if J.IsValidHero(botTarget) 
			...
			and not botTarget:IsDisarmed()
		then
			if nSkillLV >= 2 or nMP > 0.68 or J.GetHPR(botTarget) < 0.35 or nHP < 0.28
			then
...
function X.ConsiderR()
	...
	then
		...
		do 
			...
			then
				targetLocation = J.GetUnitTowardDistanceLocation(nBuilding,bot,400);
				if GetUnitToLocationDistance(bot,targetLocation) < 180
				then
					targetLocation = J.GetUnitTowardDistanceLocation(bot,nBuilding,400);
				end				
